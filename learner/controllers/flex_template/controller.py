...
def action_to_string(a : game.Actions, conj_list) -> str:
    s = ""
    for (i,n) in a.increase_conj:
        if n > 0 and i >= len(conj_list):
            s += "increase_disj\n"
        else:
            for _ in range(n):
                s += f"increase_conj@{i}\n"
        ...
        def gen(msg : PCSatMsg.t, time_rest : float, current_time : float) -> ControllerMsg.t:
            ...
            if type(msg) == PCSatMsg.TemplateExtension:
                ...
                if need_restart:
                    ...
                    # msg.data is a list of tuples: (predicate_name: str, num_of_args: int, unsat_core_info: typing.Any)
                    data = copy.copy(msg.data[0][2])
                    #print("before:{}", data)
                    data.pop(1)
                    ...
                    return ControllerMsg.ActAndRead(action_to_string(act, msg.data[0]), time_rest)
