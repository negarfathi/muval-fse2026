...
<<<<<<< HEAD
                    # s0 : game.States = (time_spent, game.SAct(msg.data, msg.additional_data, z3_logs, is_restarted, msg.program))
                    # s0 : game.States = (time_spent, game.SAct(msg.data, msg.additional_data, z3_logs, is_restarted))
                    data = copy.copy(msg.data)
                    data.pop(2)
                    s0 : game.States = (time_spent, game.SAct(data, msg.additional_data, z3_logs, is_restarted))
                    # s0 : game.States = (time_spent, game.SAct(data, z3_logs, is_restarted))
=======
                    data = copy.copy(msg.data)
                    data.pop(2)
                    s0 : game.States = (time_spent, game.SAct(data, z3_logs, is_restarted))
>>>>>>> 6a70f08b3 (fixing the agents (under simple_controller) in order to follow the update of message structure from PCSat)
                    print(repr({'tag':'ACT', 'time':time_spent, 'state':s0, "rawdata": msg.data}), file=logFileObj)
