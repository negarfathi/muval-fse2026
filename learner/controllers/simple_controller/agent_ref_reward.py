...
def reward(state : game.States, t) -> float:
    s = state[1]

    if isinstance(s, game.STimeout):
        return -10.0 - s.total_time + t[pathlib.Path(s.filename).name]
...
class AgentRefReward(AgentABC):
    def __init__(self, coreClass, coreConfig, reward_table):
