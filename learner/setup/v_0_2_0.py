...
import run
import learner_core
from controllers.simple_controller.agent import Agent, PCSatAgent
from controllers.simple_controller.agent_ref_reward import AgentRefReward
from controllers.simple_controller.agent_conj_biased import ConjBiasedAgent, ConjBiasedPCSatAgent
from controllers.simple_controller.agent_neutral import UnbiasedAgent
from controllers.simple_controller.agent_neutral_finer_abstraction import UnbiasedFinerAgent
from controllers.simple_controller.agent_noflag_nolog import AgentNoFlagNoLog
from controllers.simple_controller.agent_noflag import AgentNoFlag
from controllers.simple_controller.agent_nolog import AgentNoLog
from controllers.simple_controller.agent_neutral_noflag_nolog import AgentNeutralNoFlagNoLog
from controllers.simple_controller.agent_neutral_noflag import AgentNeutralNoFlag
from controllers.simple_controller.agent_neutral_nolog import AgentNeutralNoLog
from controllers.simple_controller.controller import Controller

def v_0_2_0_run(args, config):
    ...
    if config["agent"]["kind"] == "PCSatAgent":
        ...
        elif config["agent"]["kind"] == "Vanilla Actor Critic":
            core_config.is_learning = bool(config["agent"]["learn"])
            core_config.epsilon = float(config["agent"]["epsilon"])
            core_config.alpha = float(config["agent"]["alpha"])
            core_config.gamma = float(config["agent"]["gamma"])
            coreClass = learner_core.VanillaAC
