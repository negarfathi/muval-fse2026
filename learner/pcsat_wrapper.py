...
def read_and_act(reader,writer,controller,init_timeout,res):
    ...
    while True:
        ...
        # print("matched")
        if m:
            # num_args is the number of arguments of this predicate,
            # obtained by the previous line
            print("state str=", m[2])
            info_tuple = (m[1], num_args, json.loads(m[2]))
            preds_info.append(info_tuple)
            continue

        m = re.match(r"user time:(.*)", line)
        if m:
            user_time = float(m[1])
            continue

        m = re.match(r"end of states(.*)",line)
        if m:
            msg = PCSatMsg.TemplateExtension(copy.copy(preds_info), user_time)
            preds_info = []
