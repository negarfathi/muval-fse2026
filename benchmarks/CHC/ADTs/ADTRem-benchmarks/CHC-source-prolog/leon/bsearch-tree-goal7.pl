...
ff :-    B=:=1, A=:=0,  I-J>=1,    tcontains(X,J,A),  tinsert(X,I,F), tcontains(F,J,B).
ff :-    B=:=1, A=:=0,  I-J=< -1,  tcontains(X,J,A),  tinsert(X,I,F), tcontains(F,J,B).
ff :-    B=:=0, A=:=1,  tcontains(X,J,A),  tinsert(X,I,F), tcontains(F,J,B).
ff :-    B=:=0, I=J,  tcontains(X,J,A),  tinsert(X,I,F), tcontains(F,J,B).
