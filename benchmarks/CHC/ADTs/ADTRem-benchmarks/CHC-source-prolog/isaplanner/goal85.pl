...
% (forall ((xs Lst) (ys Lst) (zs Lst)) (= (zip (append xs ys) zs)
                    % (zappend (zip xs (take (len xs) zs)) (zip ys (drop (len xs) zs))))) ; G85


ff :-append(X,Y,XY), zip(XY,Z, Zip),
    len(X,LX),
    take(LX,Z,TZ),  zip(X,TZ,ZipX),
    drop(LX,Z,DZ),  zip(Y,DZ,ZipY),
    zappend(ZipX,ZipY,ZipXY),
    A=:=0, adt_eqlistpairs(Zip,ZipXY,A).
