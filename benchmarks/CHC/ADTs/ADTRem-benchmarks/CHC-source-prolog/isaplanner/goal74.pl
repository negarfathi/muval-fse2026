...
%ff :- A=:=0, drop(B,C,D), rev(D,E), len(C,F), minus(F,B,G), rev(C,H), take(G,H,I), adt_new1(E,I,A).
ff :- A=:=0, drop(B,C,D), rev(D,E), len(C,F), G=F-B, G>=0, rev(C,H), take(G,H,I), adt_new1(E,I,A).
ff :- A=:=0, drop(B,C,D), rev(D,E), len(C,F), G=0, F=<B-1, rev(C,H), take(G,H,I), adt_new1(E,I,A).
