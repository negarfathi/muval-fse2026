...
% ./hipspec/rotate-goal5.smt2.pl.log-new1(A,B,C) :- adt_equals(list(int),A,[B],C).
% ./hipspec/rotate-goal5.smt2.pl.log-new2(A,B) :- adt_equals(list(int),A,[],B).


% ff :- A=:=0, B>=0, rotate(B,[C],D), adt_singleton_eq(D,C,A).

ff :- A=:=0, B>=0, rotate(B,[C],D), adt_equals(D,[C],A).
