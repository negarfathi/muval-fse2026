;(set-option :produce-unsat-cores true )
(set-logic ALL_SUPPORTED )
(declare-sort A$ 0 )
(declare-sort Nat$ 0 )
(declare-sort Natural$ 0 )
(declare-sort Nat_a_fun$ 0 )
(declare-sort A_bool_fun$ 0 )
(declare-sort Bool_nat_fun$ 0 )
(declare-sort A_stream$ 0)
(declare-fun shd$ (A_stream$)A$)
(declare-fun stl$ (A_stream$)A_stream$)
(declare-fun sCons$ (A$ A_stream$ )A_stream$)
(declare-datatypes ()((Nibble$ (nibble0$ )(nibble1$ )(nibble2$ )(nibble3$ )(nibble4$ )(nibble5$ )(nibble6$ )(nibble7$ )(nibble8$ )(nibble9$ )(nibbleA$ )(nibbleB$ )(nibbleC$ )(nibbleD$ )(nibbleE$ )(nibbleF$ ))(Char$ (char$ (select$ Nibble$ )(selecta$ Nibble$ )))))
(declare-fun x$ ()A$ )
(declare-fun xs$ ()A_stream$ )
(declare-fun size$ (Char$ )Nat$ )
(declare-fun snth$ (A_stream$ )Nat_a_fun$ )
(declare-fun zero$ ()Nat$ )
(declare-fun zero$a ()Natural$ )
(declare-fun of_seq$ (Nat_a_fun$ )A_stream$ )
(declare-fun fun_app$ (Nat_a_fun$ Nat$ )A$ )
(declare-fun smember$ (A$ A_stream$ )Bool )
(declare-fun fun_app$a (Bool_nat_fun$ Bool )Nat$ )
(declare-fun fun_app$b (A_bool_fun$ A$ )Bool )
(declare-fun rec_bool$ (Nat$ Nat$ )Bool_nat_fun$ )
(declare-fun size_bool$ ()Bool_nat_fun$ )
(declare-fun size_char$ (Char$ )Nat$ )
(declare-fun pred_stream$ (A_bool_fun$ A_stream$ )Bool )
(declare-fun size_natural$ (Natural$ )Nat$ )
(assert (! (not (= (fun_app$ (snth$ (sCons$ x$ xs$ ))zero$ )x$ )):named a0 ))
(assert (! (forall ((?v0 A$ )(?v1 A_stream$ )(?v2 A$ )(?v3 A_stream$ ))(= (= (sCons$ ?v0 ?v1 )(sCons$ ?v2 ?v3 ))(and (= ?v0 ?v2 )(= ?v1 ?v3 )))):named a1 ))
(assert (! (forall ((?v0 Nat_a_fun$ ))(= (snth$ (of_seq$ ?v0 ))?v0 )):named a2 ))
(assert (! (forall ((?v0 A_stream$ ))(=> (forall ((?v1 A$ )(?v2 A_stream$ ))(=> (= ?v0 (sCons$ ?v1 ?v2 ))false ))false )):named a3 ))
(assert (! (= zero$ zero$ ):named a4 ))
(assert (! (forall ((?v0 Nat$ ))(=> (and (=> (= ?v0 zero$ )false )(=> (not (= ?v0 zero$ ))false ))false )):named a5 ))
(assert (! (forall ((?v0 Nat$ ))(= (= zero$ ?v0 )(= ?v0 zero$ ))):named a6 ))
(assert (! (forall ((?v0 Natural$ ))(= (= zero$a ?v0 )(= ?v0 zero$a ))):named a7 ))
(assert (! (forall ((?v0 Char$ ))(! (= (size_char$ ?v0 )zero$ ):pattern ((size_char$ ?v0 )))):named a8 ))
(assert (! (forall ((?v0 A$ )(?v1 A$ )(?v2 A_stream$ ))(! (= (smember$ ?v0 (sCons$ ?v1 ?v2 ))(ite (= ?v0 ?v1 )true (smember$ ?v0 ?v2 ))):pattern ((smember$ ?v0 (sCons$ ?v1 ?v2 ))))):named a9 ))
(assert (! (= (fun_app$a size_bool$ true )zero$ ):named a10 ))
(assert (! (= (fun_app$a size_bool$ false )zero$ ):named a11 ))
(assert (! (= size_bool$ (rec_bool$ zero$ zero$ )):named a12 ))
(assert (! (= (size_natural$ zero$a )zero$ ):named a13 ))
(assert (! (forall ((?v0 Char$ ))(! (= (size$ ?v0 )zero$ ):pattern ((size$ ?v0 )))):named a14 ))
(assert (! (forall ((?v0 A_bool_fun$ )(?v1 A$ )(?v2 A_stream$ ))(! (= (pred_stream$ ?v0 (sCons$ ?v1 ?v2 ))(and (fun_app$b ?v0 ?v1 )(pred_stream$ ?v0 ?v2 ))):pattern ((pred_stream$ ?v0 (sCons$ ?v1 ?v2 ))))):named a15 ))
(check-sat )
;(get-unsat-core )
