...
(assert (! (not (= list_of$ (list_of_aux$ nil$ ))):named a0 ))
(assert (! (forall ((?v0 A_list$ ))(=> (and (=> (= ?v0 nil$ )false )(=> (not (= ?v0 nil$ ))false ))false )):named a1 ))
(assert (! (= (fun_app$ list_of$ lNil$ )nil$ ):named a2 ))
...
(assert (! (forall ((?v0 A_bool_fun$ ))(! (= (fun_app$a (list_ex1$ ?v0 )nil$ )false ):pattern ((list_ex1$ ?v0 )))):named a5 ))
(assert (! (forall ((?v0 A$ ))(! (= (fun_app$b (member$ nil$ )?v0 )false ):pattern ((fun_app$b (member$ nil$ )?v0 )))):named a6 ))
(assert (! (forall ((?v0 Nat$ ))(! (= (fun_app$c (gen_length$ ?v0 )nil$ )?v0 ):pattern ((gen_length$ ?v0 )))):named a7 ))
(assert (! (forall ((?v0 A_list$ ))(! (= (fun_app$d (splice$ ?v0 )nil$ )?v0 ):pattern ((splice$ ?v0 )))):named a8 ))
(assert (! (forall ((?v0 A_a_list_fun$ ))(! (= (fun_app$d (maps$ ?v0 )nil$ )nil$ ):pattern ((maps$ ?v0 )))):named a9 ))
...
(assert (! (= (llist_of$ nil$ )lNil$ ):named a14 ))
(assert (! (forall ((?v0 A_list$ ))(= (= (llist_of$ ?v0 )lNil$ )(= ?v0 nil$ ))):named a15 ))
(assert (! (= (rotate1$ nil$ )nil$ ):named a16 ))
(assert (! (forall ((?v0 A_list$ ))(! (= (fun_app$d (splice$ nil$ )?v0 )?v0 ):pattern ((fun_app$d (splice$ nil$ )?v0 )))):named a17 ))
(check-sat )
;(get-unsat-core )
