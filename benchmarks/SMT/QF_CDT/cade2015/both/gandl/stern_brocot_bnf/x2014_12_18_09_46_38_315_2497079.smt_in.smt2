...
(assert (! (less$a zero$ (snd$ (mediant$ (mulmat$ (mulmat$ x$ uRmat$) y$)))) :named a12))
(assert (! (= (det$ uRmat$) one$) :named a13))
(assert (! (= uRmat$ (pair$a (pair$ one$ zero$) (pair$ one$ one$))) :named a14))
(assert (! (forall ((?v0 Nat_nat_prod_nat_nat_prod_prod$)) (=> (= (det$ ?v0) one$) (= (det$ (mulmat$ ?v0 uRmat$)) one$))) :named a15))
(assert (! (forall ((?v0 Nat_nat_prod_nat_nat_prod_prod$)) (=> (= (det$ ?v0) one$) (= (det$ (mulmat$ uRmat$ ?v0)) one$))) :named a16))
(assert (! (forall ((?v0 Nat_nat_prod_nat_nat_prod_prod$)) (=> (less$a zero$ (snd$ (mediant$ ?v0))) (less$a zero$ (snd$ (mediant$ (mulmat$ ?v0 uRmat$)))))) :named a17))
...
(assert (! (= (mediant$ fmat$) (pair$ one$ one$)) :named a21))
(assert (! (forall ((?v0 Nat_nat_prod_nat_nat_prod_prod$) (?v1 Nat_nat_prod_nat_nat_prod_prod$)) (=> (and (= (det$ ?v0) one$) (and (= (det$ ?v1) one$) (less$a zero$ (snd$ (mediant$ ?v0))))) (less$ (case_prod$ uu$ (mediant$ (mulmat$ (mulmat$ ?v0 lLmat$) ?v1))) (case_prod$ uu$ (mediant$ ?v0))))) :named a22))
(assert (! (= (of_nat$a one$) one$a) :named a23))
(assert (! (= (of_nat$b one$) one$) :named a24))
(assert (! (= (of_nat$ one$) 1) :named a25))
(assert (! (forall ((?v0 Nat$)) (= (less$a ?v0 one$) (= ?v0 zero$))) :named a26))
(assert (! (forall ((?v0 Nat$) (?v1 Nat$)) (= (less$ (of_nat$a ?v0) (of_nat$a ?v1)) (less$a ?v0 ?v1))) :named a27))
(assert (! (forall ((?v0 Nat$) (?v1 Nat$)) (= (less$a (of_nat$b ?v0) (of_nat$b ?v1)) (less$a ?v0 ?v1))) :named a28))
(assert (! (forall ((?v0 Nat$) (?v1 Nat$)) (= (< (of_nat$ ?v0) (of_nat$ ?v1)) (less$a ?v0 ?v1))) :named a29))
(assert (! (forall ((?v0 Nat$)) (= (= (of_nat$a ?v0) zero$a) (= ?v0 zero$))) :named a30))
(assert (! (forall ((?v0 Nat$)) (= (= (of_nat$b ?v0) zero$) (= ?v0 zero$))) :named a31))
(assert (! (forall ((?v0 Nat$)) (= (= (of_nat$ ?v0) 0) (= ?v0 zero$))) :named a32))
(check-sat)
;(get-unsat-core)
