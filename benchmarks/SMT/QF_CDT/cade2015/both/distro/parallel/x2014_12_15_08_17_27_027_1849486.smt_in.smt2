...
(assert (! (not (exists ((?v0 Dtree$) (?v1 Dtree$)) (and (= (par$ (pair$ tr1$ tr2$)) (par$ (pair$ ?v0 ?v1))) (= trB$ (par$ (pair$ ?v1 ?v0)))))) :named a0))
(assert (! (= trB$ (par$ (pair$ tr2$ tr1$))) :named a1))
(assert (! (member$ (inr$ tr1$) (cont$ tr1a$)) :named a2))
(assert (! (member$ (inr$ tr2$) (cont$ tr2a$)) :named a3))
(assert (! (exists ((?v0 Dtree$) (?v1 Dtree$)) (and (= trA$ (par$ (pair$ ?v0 ?v1))) (= trB$a (par$ (pair$ ?v1 ?v0))))) :named a4))
(assert (! (member$ (inr$ tr1$) (cont$ tr1a$)) :named a5))
(assert (! (member$ (inr$ tr2$) (cont$ tr2a$)) :named a6))
(assert (! (= trB$ (par$ (pair$ tr2$ tr1$))) :named a7))
(assert (! (member$ (inr$ trB$) (cont$ (par$ (pair$ tr2a$ tr1a$)))) :named a8))
(assert (! (=> (forall ((?v0 Dtree$) (?v1 Dtree$)) (=> (and (= trB$ (par$ (pair$ ?v1 ?v0))) (and (member$ (inr$ ?v0) (cont$ tr1a$)) (member$ (inr$ ?v1) (cont$ tr2a$)))) false)) false) :named a9))
