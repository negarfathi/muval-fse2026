...
(assert (! (not (= (member$ (pair$ x$ y$) (set$ (entries$ (balance_right$ l$ k$ v$ r$a)))) (or (member$ (pair$ x$ y$) (set$ (entries$ l$))) (or (and (= x$ k$) (= y$ v$)) (member$ (pair$ x$ y$) (set$ (entries$ r$a))))))) :named a0))
(assert (! (inv1l$ r$a) :named a1))
(assert (! (inv1$ l$) :named a2))
(assert (! (inv2$ l$) :named a3))
(assert (! (inv2$ r$a) :named a4))
(assert (! (= (bheight$ l$) (plus$ (bheight$ r$a) one$)) :named a5))
...
(assert (! (forall ((?v0 A_b_rbt$) (?v1 A_b_rbt$) (?v2 A$) (?v3 B$)) (=> (and (inv1$ ?v0) (inv1l$ ?v1)) (inv1l$ (balance_right$ ?v0 ?v2 ?v3 ?v1)))) :named a11))
(assert (! (forall ((?v0 A$) (?v1 B$) (?v2 A_b_rbt$) (?v3 A$) (?v4 B$) (?v5 A_b_rbt$)) (= (member$ (pair$ ?v0 ?v1) (set$ (entries$ (balance$ ?v2 ?v3 ?v4 ?v5)))) (or (member$ (pair$ ?v0 ?v1) (set$ (entries$ ?v2))) (or (and (= ?v0 ?v3) (= ?v1 ?v4)) (member$ (pair$ ?v0 ?v1) (set$ (entries$ ?v5))))))) :named a12))
(assert (! (forall ((?v0 A_b_prod$)) (=> (forall ((?v1 A$) (?v2 B$)) (=> (= ?v0 (pair$ ?v1 ?v2)) false)) false)) :named a13))
(check-sat)
;(get-unsat-core)
