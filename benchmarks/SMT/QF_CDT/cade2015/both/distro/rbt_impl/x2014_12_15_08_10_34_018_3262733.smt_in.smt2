...
(assert (! (not (rbt_greater$ less$ v$ (rbt_del_from_left$ less$ x$ lt$ k$ y$ rt$))) :named a0))
(assert (! (rbt_greater$ less$ v$ lt$) :named a1))
(assert (! (rbt_greater$ less$ v$ rt$) :named a2))
(assert (! (fun_app$ (fun_app$a less$ v$) k$) :named a3))
