...
(assert (! (inv1l$ (combine$ b$a c$)) :named a5))
(assert (! (= (bheight$ (combine$ b$a c$)) (bheight$ b$a)) :named a6))
(assert (! (= (combine$ b$a c$) (branch$ b$ lta$ kaa$ va$ rta$)) :named a7))
(assert (! (forall ((?v0 Color$) (?v1 A_b_rbt$) (?v2 A$) (?v3 B$) (?v4 A_b_rbt$) (?v5 Color$) (?v6 A_b_rbt$) (?v7 A$) (?v8 B$) (?v9 A_b_rbt$)) (= (= (branch$ ?v0 ?v1 ?v2 ?v3 ?v4) (branch$ ?v5 ?v6 ?v7 ?v8 ?v9)) (and (= ?v0 ?v5) (and (= ?v1 ?v6) (and (= ?v2 ?v7) (and (= ?v3 ?v8) (= ?v4 ?v9))))))) :named a8))
(assert (! (inv1$ (branch$ b$ a$ ka$ x$ b$a)) :named a9))
(assert (! (inv1$ (branch$ b$ c$ s$ y$ d$)) :named a10))
(assert (! (inv2$ (branch$ b$ a$ ka$ x$ b$a)) :named a11))
(assert (! (inv2$ (branch$ b$ c$ s$ y$ d$)) :named a12))
(assert (! (= (bheight$ (branch$ b$ a$ ka$ x$ b$a)) (bheight$ (branch$ b$ c$ s$ y$ d$))) :named a13))
(assert (! (=> (and (inv2$ b$a) (and (inv2$ c$) (and (= (bheight$ b$a) (bheight$ c$)) (and (inv1$ b$a) (inv1$ c$))))) (= (member$ (pair$ k$ v$) (set$ (entries$ (combine$ b$a c$)))) (or (member$ (pair$ k$ v$) (set$ (entries$ b$a))) (member$ (pair$ k$ v$) (set$ (entries$ c$)))))) :named a14))
