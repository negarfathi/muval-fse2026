...
(assert (! (not (rbt_sorted$ less$ a$)) :named a0))
(assert (! (= (combine$ b$a c$) (branch$ b$ lta$ va$ ka$ rta$)) :named a1))
(assert (! (rbt_greater$ less$ x$ d$) :named a2))
(assert (! (rbt_less$ less$ y$ (combine$ b$a c$)) :named a3))
(assert (! (rbt_greater$ less$ x$ (combine$ b$a c$)) :named a4))
