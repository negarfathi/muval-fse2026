...
(assert (! (not (rbt_sorted$ less$ (balance$ (branch$ r$ (branch$ b$ va$ vb$ vd$ vc$) x$ w$ (branch$ r$ b$a y$ s$ c$)) z$ za$ empty$))) :named a0))
...
(assert (! (rbt_sorted$ less$ empty$) :named a26))
...
(assert (! (forall ((?v0 A$) (?v1 A$) (?v2 A_b_rbt$)) (=> (and (fun_app$ (fun_app$a less$ ?v0) ?v1) (rbt_greater$ less$ ?v1 ?v2)) (rbt_greater$ less$ ?v0 ?v2))) :named a30))
(assert (! (rbt_greater$ less$ z$ empty$) :named a31))
...
(assert (! (rbt_greater$ less$ k$ r$a) :named a34))
(assert (! (rbt_less$ less$ k$ l$) :named a35))
(check-sat)
;(get-unsat-core)
