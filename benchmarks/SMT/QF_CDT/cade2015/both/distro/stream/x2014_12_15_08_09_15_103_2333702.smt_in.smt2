...
(assert (! (= x$ (fun_app$ (snth$ s2$) n$)) :named a1))
(assert (! (=> (forall ((?v0 Nat$)) (=> (= x$ (fun_app$ (snth$ s2$) ?v0)) false)) false) :named a2))
(assert (! (member$ x$ (sset$ s2$)) :named a3))
(assert (! (= (fun_app$ (snth$ (sinterleave$ s1$ s2$)) (fun_app$a (plus$ (fun_app$a (times$ (numeral$ (bit0$ one$))) n$)) one$a)) x$) :named a4))
