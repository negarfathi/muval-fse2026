...
(assert (! (not (fun_app$ (lprefix$ ys$) xs$)) :named a4))
...
(assert (! (fun_app$ (lprefix$ xs$) ys$) :named a11))
(assert (! (not (fun_app$ (lprefix$ xs$) ys$)) :named a12))
...
(assert (! (not (less_eq$ (llength$ ys$) (llength$ xs$))) :named a15))
(assert (! (= (lappend$ xs$ xs$a) (lappend$ ys$ zs$)) :named a16))
(assert (! (=> (forall ((?v0 A_llist$)) (=> (= (lappend$ xs$ ?v0) (lappend$ ys$ zs$)) false)) false) :named a17))
(check-sat)
;(get-unsat-core)
