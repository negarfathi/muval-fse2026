...
(assert (! (not (member$ y$ (lset$ (llist_of_stream$ s$)))) :named a0))
(assert (! (member$ x$ (sset$ xs$)) :named a1))
(assert (! (member$ y$ (sset$ (stl$ s$))) :named a2))
(assert (! (member$ y$ (lset$ (llist_of_stream$ (stl$ s$)))) :named a3))
(assert (! (forall ((?v0 A_stream$) (?v1 A_stream$)) (= (= (llist_of_stream$ ?v0) (llist_of_stream$ ?v1)) (= ?v0 ?v1))) :named a4))
...
(assert (! (forall ((?v0 A_stream$)) (= (ltl$ (llist_of_stream$ ?v0)) (llist_of_stream$ (stl$ ?v0)))) :named a10))
(assert (! (forall ((?v0 A$) (?v1 A_llist$)) (=> (member$ ?v0 (lset$ ?v1)) (fun_app$ (lsetp$ ?v1) ?v0))) :named a11))
(assert (! (forall ((?v0 A_llist$) (?v1 A$)) (=> (fun_app$ (lsetp$ ?v0) ?v1) (member$ ?v1 (lset$ ?v0)))) :named a12))
...
(assert (! (forall ((?v0 A_a_bool_fun_fun$) (?v1 A_llist$)) (= (llist_all2$ ?v0 ?v1 ?v1) (forall ((?v2 A$)) (=> (member$ ?v2 (lset$ ?v1)) (fun_app$ (fun_app$a ?v0 ?v2) ?v2))))) :named a16))
