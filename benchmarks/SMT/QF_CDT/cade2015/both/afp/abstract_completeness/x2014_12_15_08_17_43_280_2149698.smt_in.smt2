...
(assert (! (forall ((?v0 State_rule_prod_stream$)) (=> (ipath$ (mkTree$ eff$ (fenum$ rules$) s$) ?v0) thesis$)) :named a1))
(assert (! (forall ((?v0 State_rule_prod_tree$)) (=> (not (tfinite$ ?v0)) (ipath$ ?v0 (konig$ ?v0)))) :named a2))
(assert (! (not (tfinite$ (mkTree$ eff$ (fenum$ rules$) s$))) :named a3))
(assert (! (member$ s$ s$a) :named a4))
(assert (! (fair$ rules$ (fenum$ rules$)) :named a5))
(assert (! (forall ((?v0 State_rule_prod_tree$) (?v1 State_rule_prod_stream$)) (=> (tfinite$ ?v0) (not (ipath$ ?v0 ?v1)))) :named a6))
...
(assert (! (forall ((?v0 State_rule_prod_tree$) (?v1 State_rule_prod_stream$)) (=> (and (wf$ eff$ rules$ ?v0) (ipath$ ?v0 ?v1)) (epath$ eff$ rules$ ?v1))) :named a9))
(assert (! (forall ((?v0 State$) (?v1 Rule_stream$) (?v2 State_rule_prod_stream$)) (=> (and (member$ ?v0 s$a) (and (fair$ rules$ ?v1) (ipath$ (mkTree$ eff$ ?v1 ?v0) ?v2))) (saturated$ eff$ rules$ ?v2))) :named a10))
(assert (! (not (exists ((?v0 State_rule_prod_tree$)) (and (= (fst$ (root$ ?v0)) s$) (and (wf$ eff$ rules$ ?v0) (tfinite$ ?v0))))) :named a11))
(assert (! (forall ((?v0 State_rule_prod_stream$)) (= (saturated$ eff$ rules$ ?v0) (forall ((?v1 Rule$)) (=> (member$a ?v1 (sset$ rules$)) (saturated$a eff$ ?v1 ?v0))))) :named a12))
(assert (! (forall ((?v0 Rule_stream$)) (fair$ ?v0 (fenum$ ?v0))) :named a13))
(assert (! (forall ((?v0 Rule$) (?v1 State$)) (=> (enabled$ eff$ ?v0 ?v1) (fun_app$ (fun_app$a (fun_app$b eff$ ?v0) ?v1) (pickEff$ eff$ ?v0 ?v1)))) :named a14))
(assert (! (forall ((?v0 State$)) (=> (member$ ?v0 s$a) (exists ((?v1 Rule$)) (and (member$a ?v1 (sset$ rules$)) (exists ((?v2 State_fset$)) (fun_app$ (fun_app$a (fun_app$b eff$ ?v1) ?v0) ?v2)))))) :named a15))
(assert (! (forall ((?v0 State$) (?v1 Rule_stream$)) (=> (and (member$ ?v0 s$a) (fair$ rules$ ?v1)) (wf$ eff$ rules$ (mkTree$ eff$ ?v1 ?v0)))) :named a16))
(assert (! (forall ((?v0 State$) (?v1 Rule_stream$) (?v2 State_rule_prod_stream$) (?v3 Rule$)) (=> (and (member$ ?v0 s$a) (and (fair$ rules$ ?v1) (and (ipath$ (mkTree$ eff$ ?v1 ?v0) ?v2) (member$a ?v3 (sset$ rules$))))) (saturated$a eff$ ?v3 ?v2))) :named a17))
(assert (! (forall ((?v0 Rule$)) (=> (member$a ?v0 (sset$ rules$)) (per$ eff$ rules$ s$a ?v0))) :named a18))
(check-sat)
;(get-unsat-core)
