; --full-saturate-quant --inst-when=full-last-call --inst-no-entail --term-db-mode=relevant --random-seed=1 --lang=smt2 --tlimit 676
...
(assert (! (not (= (height$ t_1$) zero$)) :named a1))
(assert (! (not (= (height$ t_2$) zero$)) :named a2))
...
(assert (! (=> (and (consistent$ t_2$) (and (not (= (sibling$ t_2$ b$) b$)) (not (= a$ b$)))) (= (sibling$ (fun_app$ (swapLeaves$ t_2$ w_a$ a$ w_s$) (sibling$ t_2$ b$)) a$) b$)) :named a10))
...
(assert (! (forall ((?v0 A_tree$) (?v1 A$)) (=> (consistent$ ?v0) (= (sibling$ ?v0 (sibling$ ?v0 ?v1)) ?v1))) :named a12))
(assert (! (forall ((?v0 A_tree$) (?v1 Nat$) (?v2 A$) (?v3 Nat$) (?v4 A$)) (= (height$ (fun_app$ (swapLeaves$ ?v0 ?v1 ?v2 ?v3) ?v4)) (height$ ?v0))) :named a13))
