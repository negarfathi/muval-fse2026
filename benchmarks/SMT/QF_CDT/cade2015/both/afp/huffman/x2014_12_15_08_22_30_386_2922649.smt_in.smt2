...
(assert (! (not (optimum$ (splitLeaf$ (huffman$ (insortTree$ (leaf$ (plus$ w_a$ w_b$) a$) ts$)) w_a$ a$ w_b$ b$))) :named a0))
...
(assert (! (optimum$ (huffman$ (insortTree$ (leaf$ (plus$ w_a$ w_b$) a$) ts$))) :named a5))
...
(assert (! (= (huffman$ tsa$) (huffman$ (insortTree$ (uniteTrees$ t_a$ t_b$) ts$))) :named a8))
...
(assert (! (forall ((?v0 A_tree$) (?v1 A_tree_list$)) (not (= (insortTree$ ?v0 ?v1) nil$))) :named a12))
(assert (! (forall ((?v0 A_tree$) (?v1 Nat$) (?v2 A$) (?v3 Nat$) (?v4 A$)) (= (cachedWeight$ (splitLeaf$ ?v0 ?v1 ?v2 ?v3 ?v4)) (cachedWeight$ ?v0))) :named a13))
(assert (! (forall ((?v0 A_bool_fun$) (?v1 Nat$) (?v2 A$)) (! (= (pred_tree$ ?v0 (leaf$ ?v1 ?v2)) (fun_app$ ?v0 ?v2)) :pattern ((pred_tree$ ?v0 (leaf$ ?v1 ?v2))))) :named a14))
