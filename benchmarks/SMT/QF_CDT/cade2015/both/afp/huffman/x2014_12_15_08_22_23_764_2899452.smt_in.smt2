; --full-saturate-quant --inst-when=full-last-call --inst-no-entail --term-db-mode=relevant --random-seed=1 --lang=smt2 --tlimit 327
...
(assert (! (forall ((?v0 A_tree$) (?v1 A_tree$) (?v2 A_tree_list$)) (! (= (huffman$ (cons$ ?v0 (cons$ ?v1 ?v2))) (huffman$ (fun_app$ (insortTree$ (uniteTrees$ ?v0 ?v1)) ?v2))) :pattern ((cons$ ?v0 (cons$ ?v1 ?v2))))) :named a9))
...
(assert (! (= ts$ (cons$ t_b$ ts$a)) :named a13))
(assert (! (forall ((?v0 A$) (?v1 A_tree$)) (! (=> (not (member$ ?v0 (alphabet$ ?v1))) (= (fun_app$a (freq$ ?v1) ?v0) zero$)) :pattern ((fun_app$a (freq$ ?v1) ?v0)))) :named a14))
(assert (! (forall ((?v0 A$) (?v1 A_tree_list$)) (! (=> (not (member$ ?v0 (alphabet_F$ ?v1))) (= (fun_app$a (freq_F$ ?v1) ?v0) zero$)) :pattern ((fun_app$a (freq_F$ ?v1) ?v0)))) :named a15))
(assert (! (forall ((?v0 A_tree$) (?v1 A_tree_list$)) (= (consistent_F$ (fun_app$ (insortTree$ ?v0) ?v1)) (consistent_F$ (cons$ ?v0 ?v1)))) :named a16))
(assert (! (forall ((?v0 A_tree$)) (! (= (fun_app$ (insortTree$ ?v0) nil$) (cons$ ?v0 nil$)) :pattern ((insortTree$ ?v0)))) :named a17))
(assert (! (forall ((?v0 A$)) (! (= (fun_app$a (freq_F$ nil$) ?v0) zero$) :pattern ((fun_app$a (freq_F$ nil$) ?v0)))) :named a18))
(check-sat)
;(get-unsat-core)
