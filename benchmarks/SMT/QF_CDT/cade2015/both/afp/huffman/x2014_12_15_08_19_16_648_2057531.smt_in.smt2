...
(assert (! (not (= (sibling$ t$ b$) a$)) :named a0))
(assert (! (consistent$ t$) :named a1))
(assert (! (= (sibling$ t$ a$) b$) :named a2))
...
(assert (! (= (consistent_F$ nil$) true) :named a17))
(assert (! (forall ((?v0 Nat$) (?v1 A$) (?v2 A$)) (! (= (depth$ (leaf$ ?v0 ?v1) ?v2) zero$) :pattern ((depth$ (leaf$ ?v0 ?v1) ?v2)))) :named a18))
(assert (! (forall ((?v0 Nat$) (?v1 A$)) (! (= (height$ (leaf$ ?v0 ?v1)) zero$) :pattern ((leaf$ ?v0 ?v1)))) :named a19))
(check-sat)
;(get-unsat-core)
