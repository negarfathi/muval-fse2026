...
(assert (! (not (= s$ leaf$)) :named a4))
(assert (! (<= 0.0 (log$ 2.0 (+ 1.0 (real$ (size$ s$))))) :named a5))
(assert (! (bst$ s$) :named a6))
(assert (! (<= (- (+ (real$ (t_splay$ a$ s$)) (phi$ (splay$a a$ s$))) (phi$ s$)) (+ (* 3.0 (log$ 2.0 (real$ (size1$ s$)))) 1.0)) :named a7))
...
(assert (! (forall ((?v0 Num$)) (= (= (bit1$ ?v0) one$) false)) :named a18))
(assert (! (forall ((?v0 Num$)) (= (= one$ (bit1$ ?v0)) false)) :named a19))
(assert (! (= e$ a$) :named a20))
(check-sat)
;(get-unsat-core)
