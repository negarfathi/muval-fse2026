; --full-saturate-quant --inst-when=full-last-call --inst-no-entail --term-db-mode=relevant --random-seed=1 --lang=smt2 --tlimit 341
...
(assert (! (= aa$ c$) :named a1))
(assert (! (bst$ (node$ l$ c$ r$)) :named a2))
(assert (! (member$ (node$ la$ aa$ ra$) (subtrees$ (node$ l$ c$ r$))) :named a3))
(assert (! (forall ((?v0 A$) (?v1 A_tree$) (?v2 A_tree$)) (! (= (a$ ?v0 (node$ ?v1 ?v0 ?v2)) 1.0) :pattern ((node$ ?v1 ?v0 ?v2)))) :named a4))
...
(assert (! (forall ((?v0 Num$)) (= (= (bit1$ ?v0) one$) false)) :named a15))
(assert (! (forall ((?v0 Num$)) (= (= one$ (bit1$ ?v0)) false)) :named a16))
(check-sat)
;(get-unsat-core)
