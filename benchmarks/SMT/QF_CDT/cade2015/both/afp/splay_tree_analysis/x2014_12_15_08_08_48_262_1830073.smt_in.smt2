...
(assert (! (forall ((?v0 A$) (?v1 A_tree$) (?v2 A_tree$)) (! (= (a$ ?v0 (node$ ?v1 ?v0 ?v2)) 1.0) :pattern ((node$ ?v1 ?v0 ?v2)))) :named a1))
(assert (! (member$ (node$ la$ aa$ ra$) (subtrees$ (node$ l$ c$ r$))) :named a2))
(assert (! (bst$ (node$ l$ c$ r$)) :named a3))
(assert (! (= (+ 1.0 1.0) 2.0) :named a4))
(assert (! (forall ((?v0 Num$)) (= (+ (numeral$ ?v0) 1.0) (numeral$ (plus$ ?v0 one$)))) :named a5))
(assert (! (forall ((?v0 Num$)) (= (+ 1.0 (numeral$ ?v0)) (numeral$ (plus$ one$ ?v0)))) :named a6))
(assert (! (forall ((?v0 Num$)) (= (<= (numeral$ ?v0) 1.0) (less_eq$ ?v0 one$))) :named a7))
