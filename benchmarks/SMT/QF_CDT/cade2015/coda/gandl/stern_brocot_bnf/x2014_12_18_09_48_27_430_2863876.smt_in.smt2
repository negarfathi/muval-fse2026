...
(assert (! (not (= (root$ den$ )one$ )):named a0 ))
(assert (! (= (root$ num$ )one$ ):named a1 ))
(assert (! (= one$ one$ ):named a2 ))
(assert (! (forall ((?v0 Nat$ ))(= (= one$ ?v0 )(= ?v0 one$ ))):named a3 ))
(assert (! (forall ((?v0 Nat_nat_fun$ )(?v1 Nat_nat_fun$ )(?v2 Nat$ ))(= (root$ (tree_iterate$ ?v0 ?v1 ?v2 ))?v2 )):named a4 ))
(assert (! (forall ((?v0 Nat$ ))(= (root$ (tree_pure$ ?v0 ))?v0 )):named a5 ))
(assert (! (forall ((?v0 Nat_nat_fun$ )(?v1 Nat_tree$ ))(= (root$ (map_tree$ ?v0 ?v1 ))(fun_app$ ?v0 (root$ ?v1 )))):named a6 ))
(assert (! (forall ((?v0 Nat_nat_fun$ )(?v1 Nat_tree$ ))(= (root$ (map_tree$ ?v0 ?v1 ))(fun_app$ ?v0 (root$ ?v1 )))):named a7 ))
(assert (! (= (of_nat$ one$ )one$a ):named a8 ))
(assert (! (= (of_nat$a one$ )one$ ):named a9 ))
