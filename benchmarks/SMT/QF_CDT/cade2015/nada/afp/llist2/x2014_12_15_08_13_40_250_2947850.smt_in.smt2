;(set-option :produce-unsat-cores true )
(set-logic ALL_SUPPORTED )
(declare-sort A$ 0 )
(declare-sort Nat$ 0 )
(declare-sort A_set$ 0 )
(declare-sort A_bool_fun$ 0 )
(declare-sort A_llist_set$ 0 )
(declare-sort A_llist_a_llist_set_fun$ 0 )
(declare-sort A_llist$ 0)
(declare-fun lNil$ ()A_llist$)
(declare-fun lhd$ (A_llist$)A$)
(declare-fun ltl$ (A_llist$)A_llist$)
(declare-fun lCons$ (A$ A_llist$ )A_llist$)
(declare-fun a$ ()A_set$ )
(declare-fun s$ ()A_llist$ )
(declare-fun lrev$ (A_llist$ )A_llist$ )
(declare-fun suff$ (A_set$ )A_llist_a_llist_set_fun$ )
(declare-fun ldrop$ (A_llist$ Nat$ )A_llist$ )
(declare-fun ltake$ (A_llist$ Nat$ )A_llist$ )
(declare-fun lconst$ (A$ )A_llist$ )
(declare-fun member$ (A_llist$ A_llist_set$ )Bool )
(declare-fun alllsts$ (A_set$ )A_llist_set$ )
(declare-fun finlsts$ (A_set$ )A_llist_set$ )
(declare-fun fun_app$ (A_llist_a_llist_set_fun$ A_llist$ )A_llist_set$ )
(declare-fun llength$ (A_llist$ )Nat$ )
(declare-fun poslsts$ (A_set$ )A_llist_set$ )
(declare-fun alllstsp$ (A_bool_fun$ A_llist$ )Bool )
(declare-fun finlstsp$ (A_bool_fun$ A_llist$ )Bool )
(declare-fun lbutlast$ (A_llist$ )A_llist$ )
(assert (! (not (= (member$ lNil$ (fun_app$ (suff$ a$ )s$ ))(= s$ lNil$ ))):named a0 ))
(assert (! (= (lbutlast$ lNil$ )lNil$ ):named a1 ))
(assert (! (forall ((?v0 A_set$ ))(! (= (fun_app$ (suff$ ?v0 )lNil$ )(alllsts$ ?v0 )):pattern ((suff$ ?v0 )))):named a2 ))
(assert (! (forall ((?v0 A$ ))(not (= (lconst$ ?v0 )lNil$ ))):named a3 ))
(assert (! (= (lrev$ lNil$ )lNil$ ):named a4 ))
(assert (! (forall ((?v0 Nat$ ))(! (= (ltake$ lNil$ ?v0 )lNil$ ):pattern ((ltake$ lNil$ ?v0 )))):named a5 ))
(assert (! (forall ((?v0 Nat$ ))(! (= (ldrop$ lNil$ ?v0 )lNil$ ):pattern ((ldrop$ lNil$ ?v0 )))):named a6 ))
(assert (! (forall ((?v0 A_bool_fun$ ))(finlstsp$ ?v0 lNil$ )):named a7 ))
(assert (! (forall ((?v0 A_bool_fun$ ))(alllstsp$ ?v0 lNil$ )):named a8 ))
(assert (! (forall ((?v0 A_llist$ )(?v1 A_set$ )(?v2 A_llist$ ))(=> (member$ ?v0 (fun_app$ (suff$ ?v1 )?v2 ))(member$ ?v0 (alllsts$ ?v1 )))):named a9 ))
(assert (! (forall ((?v0 A_llist$ )(?v1 Nat$ ))(=> (not (= (ldrop$ ?v0 ?v1 )lNil$ ))(not (= ?v0 lNil$ )))):named a10 ))
(assert (! (forall ((?v0 A_llist$ )(?v1 A_set$ )(?v2 Nat$ ))(=> (member$ ?v0 (alllsts$ ?v1 ))(member$ (ldrop$ ?v0 ?v2 )(alllsts$ ?v1 )))):named a11 ))
(assert (! (forall ((?v0 A_set$ ))(member$ lNil$ (alllsts$ ?v0 ))):named a12 ))
(assert (! (forall ((?v0 A_llist$ )(?v1 A_set$ ))(= (member$ ?v0 (poslsts$ ?v1 ))(and (member$ ?v0 (alllsts$ ?v1 ))(not (= ?v0 lNil$ ))))):named a13 ))
(assert (! (forall ((?v0 A_llist$ )(?v1 Nat$ ))(=> (not (= (ldrop$ ?v0 ?v1 )lNil$ ))(= (llength$ (ltake$ ?v0 ?v1 ))?v1 ))):named a14 ))
(assert (! (forall ((?v0 A_llist$ )(?v1 A_set$ )(?v2 Nat$ ))(=> (member$ ?v0 (alllsts$ ?v1 ))(member$ (ltake$ ?v0 ?v2 )(finlsts$ ?v1 )))):named a15 ))
(check-sat )
;(get-unsat-core )
