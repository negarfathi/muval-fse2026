...
(assert (! (not (member$ a$ (alphabet$ u$ ))):named a2 ))
(assert (! (consistent_F$ (cons$ u$ us$ )):named a3 ))
(assert (! (member$ a$ (alphabet_F$ (cons$ u$ us$ ))):named a4 ))
(assert (! (= (freq_F$ (cons$ u$ us$ )a$ )(plus$ w_a$ w_b$ )):named a5 ))
(assert (! (=> (and (member$ a$ (alphabet_F$ us$ ))(and (consistent_F$ us$ )(and (not (member$ a$ (alphabet$ t$ )))(= (freq_F$ us$ a$ )(plus$ w_a$ w_b$ )))))(= (splitLeaf_F$ (fun_app$ (insortTree$ t$ )us$ )w_a$ a$ w_b$ b$ )(fun_app$ (insortTree$ t$ )(splitLeaf_F$ us$ w_a$ a$ w_b$ b$ )))):named a6 ))
(assert (! (forall ((?v0 A_tree$ )(?v1 A_tree_list$ ))(= (consistent_F$ (fun_app$ (insortTree$ ?v0 )?v1 ))(consistent_F$ (cons$ ?v0 ?v1 )))):named a7 ))
(assert (! (forall ((?v0 A$ )(?v1 A_tree_list$ )(?v2 Nat$ )(?v3 Nat$ )(?v4 A$ ))(! (=> (not (member$ ?v0 (alphabet_F$ ?v1 )))(= (splitLeaf_F$ ?v1 ?v2 ?v0 ?v3 ?v4 )?v1 )):pattern ((splitLeaf_F$ ?v1 ?v2 ?v0 ?v3 ?v4 )))):named a8 ))
...
(assert (! (forall ((?v0 A_tree$ )(?v1 A_tree_list$ ))(not (= (cons$ ?v0 ?v1 )?v1 ))):named a11 ))
(assert (! (forall ((?v0 A_tree$ ))(! (= (fun_app$ (insortTree$ ?v0 )nil$ )(cons$ ?v0 nil$ )):pattern ((insortTree$ ?v0 )))):named a12 ))
(assert (! (forall ((?v0 A_tree$ )(?v1 A_tree$ )(?v2 A_tree_list$ ))(! (= (huffman$ (cons$ ?v0 (cons$ ?v1 ?v2 )))(huffman$ (fun_app$ (insortTree$ (uniteTrees$ ?v0 ?v1 ))?v2 ))):pattern ((cons$ ?v0 (cons$ ?v1 ?v2 ))))):named a13 ))
(assert (! (forall ((?v0 A_tree$ )(?v1 A_tree_list$ ))(=> (sortedByWeight$ (cons$ ?v0 ?v1 ))(sortedByWeight$ ?v1 ))):named a14 ))
(assert (! (forall ((?v0 Nat$ )(?v1 A$ )(?v2 Nat$ )(?v3 A$ ))(! (= (splitLeaf_F$ nil$ ?v0 ?v1 ?v2 ?v3 )nil$ ):pattern ((splitLeaf_F$ nil$ ?v0 ?v1 ?v2 ?v3 )))):named a15 ))
(assert (! (forall ((?v0 A_tree$ )(?v1 A_tree$ )(?v2 A_tree_list$ ))(! (= (fun_app$ (insortTree$ ?v0 )(cons$ ?v1 ?v2 ))(ite (less_eq$ (cachedWeight$ ?v0 )(cachedWeight$ ?v1 ))(cons$ ?v0 (cons$ ?v1 ?v2 ))(cons$ ?v1 (fun_app$ (insortTree$ ?v0 )?v2 )))):pattern ((fun_app$ (insortTree$ ?v0 )(cons$ ?v1 ?v2 ))))):named a16 ))
(check-sat )
;(get-unsat-core )
