...
(assert (! (forall ((?v0 Rule_stream$ ))(=> (fair$ rules$ ?v0 )(fair$ rules$ (stl$ ?v0 )))):named a8 ))
(assert (! (forall ((?v0 State$ )(?v1 Rule_stream$ ))(=> (and (member$ ?v0 s$a )(fair$ rules$ ?v1 ))(fair$ rules$ (trim$ eff$ ?v1 ?v0 )))):named a9 ))
(assert (! (enabled$ eff$ r$ sa$ ):named a10 ))
(assert (! (fair$ rules$ (fenum$ rules$ )):named a11 ))
(assert (! (forall ((?v0 State_rule_prod_stream$ )(?v1 State_rule_prod_stream$ ))(=> (fair$a ?v0 ?v1 )(fair$a ?v0 (stl$a ?v1 )))):named a12 ))
(assert (! (forall ((?v0 Rule_stream$ )(?v1 Rule_stream$ ))(=> (fair$ ?v0 ?v1 )(fair$ ?v0 (stl$ ?v1 )))):named a13 ))
(assert (! (member$a r$ (sset$ rules$ )):named a14 ))
(assert (! (not (= (pos$ rsa$ r$ )(minWait$ eff$ rsa$ sa$ ))):named a15 ))
(assert (! (forall ((?v0 Rule$ )(?v1 State$ ))(=> (enabled$ eff$ ?v0 ?v1 )(fun_app$ (fun_app$a (fun_app$b eff$ ?v0 )?v1 )(pickEff$ eff$ ?v0 ?v1 )))):named a16 ))
(assert (! (ipath$ (mkTree$ eff$ (stl$ (trim$ eff$ rsa$ sa$ ))s$b )(stl$a stepsa$ )):named a17 ))
(assert (! (forall ((?v0 Rule_stream$ )(?v1 Nat$ ))(=> (fair$ rules$ ?v0 )(fair$ rules$ (sdrop$ ?v1 ?v0 )))):named a18 ))
(check-sat )
;(get-unsat-core )
