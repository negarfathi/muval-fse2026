...
[@@@assert "typeof(nth) <: (t1:{l: int list | l <> []}) -> (t2:{n : int | n = 1 }) -> {ret : int | $hd(t1) = ret}"]
...
let rec iteri_aux i (f: int -> 'a -> unit) = function
    [] -> ()
  | a::l -> f i a; iteri_aux (i + 1) f l

let iteri (f: int -> 'a -> unit) l = iteri_aux 0 f l
...
let rec iter2 (f: 'a -> 'b -> unit) l1 l2 =
  match (l1, l2) with
    ([], []) -> ()
