...
let main () =
  try_with (fun () ->
    let x = if perform (Decide (0, 0)) then 10 else 20 in
    let y = if perform (Decide (x, 0)) then 1 else 2 in
    let z = if perform (Decide (x, y)) then 1 else 2 in
    x - y - z
  ) () {
