type 'a eff =
| Pick : int * int(*ctxinfo*) -> int eff
type exn =
| Fail of int(*ctxinfo*) * bool(*ctxinfo*)
...
let rec find_solution n col : int list =
  if col = 0 then []
    ...
    let queen = perform (Pick (n, col)) in
    let is_safe = safe queen 1 sol in
    if is_safe then queen::sol else raise (Fail (col, is_safe))
  end

let queens_count n =
  match_with (find_solution n) n {
    retc = (fun _ -> 1);
    exnc = (fun (Fail (_ctx1, _ctx2)) -> 0);
    effc = fun (type b) (e : b eff) ->
      match e with
      | Pick (n, _ctx) -> Some (fun (k : (b, _) continuation) ->
...
[@@@assert "typeof(queens_count) <: (n: {z: int | z = 4}) -> {z: int | z = 2}"]
