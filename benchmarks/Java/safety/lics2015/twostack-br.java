...
class Queue extends Object {
  ...
  Object dequeue() { 
    if_ then return dequeue: this.refill() else return dequeue: new Queue(this.in, this.out.pop());
  }
}
main() {
  if_ then
    return new Queue(new Empty(), new Cons(new String(),new Empty())).enqueue(new String()).dequeue()
  else 
  return new Queue(new Cons(new String(), new Empty()), new Cons(new String(),new Empty())).dequeue().enqueue(new String());
}
