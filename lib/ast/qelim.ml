...
let elim_consts bounds (uni_senv, defs, phi) =
  let cm, phis1 =
    let dijuncts = List.map (BoolTerm.disjuncts_of phi) ~f:BoolTerm.nnf_of in
    List.partition_map dijuncts ~f:(consts_of bounds)
  in
  try
    let sub =
...
let rec qelim_aux2 let_bounds bounds exi_senv (uni_senv, defs, phi) =
  ...
  let dijuncts = List.map (BoolTerm.disjuncts_of phi) ~f:BoolTerm.nnf_of in
  let res =
    List.find_map dijuncts ~f:(fun phi ->
        match Term.let_apps phi with
            ...
            | ( t2,
              ...
              when (not (Map.Poly.mem bounds' x))
                   && has_no_inter x
                        (Set.union (Term.fvs_of t1) (Term.fvs_of t2)) ->
                Some (x, Term.mk_sym_app (IntTerm.neg_sym s) [ t2; t1 ])
            | App (App (Con ((IntTerm.Add as s), _), t1, _), Var (x, _), _), t2
              when (not (Map.Poly.mem bounds' x))
                   && has_no_inter x
                        (Set.union (Term.fvs_of t1) (Term.fvs_of t2)) ->
                Some (x, Term.mk_sym_app (IntTerm.neg_sym s) [ t2; t1 ])
            | t2, App (App (Con ((IntTerm.Add as s), _), t1, _), Var (x, _), _)
              when (not (Map.Poly.mem bounds' x))
                   && has_no_inter x
                        (Set.union (Term.fvs_of t1) (Term.fvs_of t2)) ->
                Some (x, Term.mk_sym_app (IntTerm.neg_sym s) [ t2; t1 ])
            | App (App (Con ((IntTerm.Sub as s), _), t1, _), Var (x, _), _), t2
              when (not (Map.Poly.mem bounds' x))
                   && has_no_inter x
                        (Set.union (Term.fvs_of t1) (Term.fvs_of t2)) ->
            ...
            | ( App (App (Con ((RealTerm.RAdd as s), _), t1, _), Var (x, _), _),
              ...
              when (not (Map.Poly.mem bounds' x))
                   && has_no_inter x
                        (Set.union (Term.fvs_of t1) (Term.fvs_of t2)) ->
                Some (x, Term.mk_sym_app (RealTerm.neg_sym s) [ t2; t1 ])
            | t2, App (App (Con ((RealTerm.RAdd as s), _), t1, _), Var (x, _), _)
              when (not (Map.Poly.mem bounds' x))
                   && has_no_inter x
                        (Set.union (Term.fvs_of t1) (Term.fvs_of t2)) ->
                Some (x, Term.mk_sym_app (RealTerm.neg_sym s) [ t2; t1 ])
            | ( App (App (Con ((RealTerm.RSub as s), _), t1, _), Var (x, _), _),
              ...
              when (not (Map.Poly.mem bounds' x))
                   && has_no_inter x
                        (Set.union (Term.fvs_of t1) (Term.fvs_of t2)) ->
                Some (x, Term.mk_sym_app s [ t1; t2 ])
            | t2, App (App (Con ((RealTerm.RSub as s), _), t1, _), Var (x, _), _)
              when (not (Map.Poly.mem bounds' x))
                   && has_no_inter x
                        (Set.union (Term.fvs_of t1) (Term.fvs_of t2)) ->
                Some (x, Term.mk_sym_app s [ t1; t2 ])
  ...
  let res =
    ...
    | Some (x, t) -> Some (x, t)
    | None ->
        List.filter_map dijuncts ~f:(fun phi ->
            match
              List.map (BoolTerm.conjuncts_of phi) ~f:(consts_of Map.Poly.empty)
            with
            | [
  ...
  | Some (x, t) ->
      ...
      qelim_aux2 let_bounds bounds exi_senv
        (uni_senv', List.map ~f:(Term.subst sub) defs, Term.subst sub phi)
  | None -> (uni_senv, defs, phi)

let app_qelim let_bounds bounds exi_senv phi =
  let uni_senv, defs, phi1 = qelim_aux1 let_bounds bounds exi_senv phi in
  let phi2 =
    ExtTerm.simplify_formula exi_senv (Map.force_merge let_bounds uni_senv) phi1
  in
  let uni_senv, defs, phi3 =
    qelim_aux2 let_bounds bounds exi_senv (uni_senv, defs, phi2)
  in
  let phi4 =
    ExtTerm.simplify_formula exi_senv (Map.force_merge let_bounds uni_senv) phi3
  in
  (uni_senv, defs, phi4)
