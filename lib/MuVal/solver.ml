...
module Make (Cfg : Config.ConfigType) = struct
  ...
  let underapprox_of (pred : QMuCLP.Pred.t) =
    let cond, template =
  ...
  let con_of fsub (pred : QMuCLP.Pred.t) =
    QMuCLP.Pred.simplify
      {
        pred with
        body =
          T_real.mk_rsub
  ...
  let solve_quant ?(print_sol = false) (qmuclp : QMuCLP.Problem.t) =
    Debug.print @@ lazy "======== MuVal ========";
    let qmuclp = QMuCLP.Problem.simplify qmuclp in
    Debug.print ~id:None
    @@ lazy (sprintf "input: %s\n" @@ QMuCLP.Problem.str_of qmuclp);
    if
      List.exists qmuclp.preds ~f:(fun pred ->
          Stdlib.( = ) Predicate.Nu pred.kind)
    then failwith "not implemented";
    let (ua_preds, prefp, rank, postfp), pqes =
      match qmuclp.query.kind with
      | LB ->
          let conds, templ_paramss, templ_constrss, ua_preds =
            List.unzip4 @@ List.map qmuclp.preds ~f:underapprox_of
          in
          Debug.print ~id:None
          @@ lazy
               (sprintf "\nunderapproximated:\n%s\n"
               @@ QMuCLP.Pred.str_of_list ua_preds);
          let prefp_templ_paramss, prefp_templ_preds =
            List.unzip
            @@ List.map2_exn qmuclp.preds conds ~f:(fun pred cond ->
                   let template =
                         ...
                         let templ_params, quant_pred =
                           if false then
                             let templ =
                               Templ.mk_affine
                               @@ Templ.terms_of
                                    config.quant_prefp_templ_term_degree
                                    pred.args
                             in
          ...
          Debug.print ~id:None
          @@ lazy (sprintf "con:\n%s\n" @@ QMuCLP.Pred.str_of_list ua_preds_con);
          let constrs =
            ...
            @ (List.map ~f:(Formula.subst_funcs postfp_sub)
              @@ QMuCLP.Problem.formula_of_query qmuclp.query
                 :: List.map ua_preds ~f:(fun pred ->
                        Formula.geq
                          (Term.elim_min ~pos:true pred.body)
            ...
            @@ List.map ua_preds ~f:(fun pred ->
                   Formula.geq
                     ...
                     (Term.elim_min ~pos:false pred.body))
                    ...
                    (T_real.mk_radd
                       (Term.rename
                          (LogicOld.ren_of_sort_env_list ua_preds_con.args
                             rank_args)
                          (Term.elim_min ~pos:false ua_preds_con.body))
                       (Term.rename
      ...
      | UB ->
          let prefp_templ_paramss, prefp_templ_preds =
            ...
            @@ List.map qmuclp.preds ~f:(fun pred ->
                   ...
                   in
                   ( template.templ_params,
                     (pred.name, (pred.args, template.quant_pred)) ))
          in
          if not @@ List.is_empty prefp_templ_preds then (
            Debug.print @@ lazy "templates for prefixpoint:";
            print_templ prefp_templ_preds);
          ...
          let constrs =
            ...
            @@ QMuCLP.Problem.formula_of_query qmuclp.query
               :: List.map qmuclp.preds ~f:(fun pred ->
                      Formula.geq
                        (Term.fvar_app_of_senv pred.name pred.args T_real.SReal)
                        (Term.elim_min ~pos:true (*ToDo*) pred.body))
          in
