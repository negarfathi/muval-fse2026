  ...
  let pcsp =
    if config.normalize_pcsp then (
      ...
      Set.Poly.map cls ~f:(fun (uni_senv, ps, ns, phi) ->
          ...
          if print_log then print_endline "begin";
          let phi =
          ...
          let uni_senv =
            ...
            uni_senv
          in
          if print_log then print_endline "normalized";
          let phi =
            ...
            phi
          in
          if print_log then print_endline "simplified";
          ...
          let uni_senv, phi =
            ...
            let uni_senv_phi, _, phi =
              Qelim.qelim bounds exi_senv (uni_senv, [], phi)
            in
            (Map.force_merge bounds uni_senv_phi, phi)
          in
              ...
              if Set.length ps + Set.length ns = 1 then phi
                ...
                @@ Set.Poly.map atms ~f:(fun atm ->
                       ...
                       if Set.disjoint fvs_phi fvs_atm then
                         ...
                         let uni_senv', _, phi' =
                           Qelim.qelim
                             (Map.Poly.filter_keys senv ~f:(Set.mem fvs_atm))
                             exi_senv (uni_senv, [], phi)
                         in
