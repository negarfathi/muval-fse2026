...
module Make
  ...
  let extracted_consts =
    if not config.extract_constants then Map.Poly.empty
      ...
      in
      Map.Poly.mapi extracted_qualifiers ~f:(fun ~key:_ ~data:_ -> consts)
      ...
      Set.fold
        ...
        ~f:(fun
          ...
          let constr =
            ...
            in
            if
              Set.exists (Formula.term_sort_env_of constr) ~f:(fun (_, s) ->
                  Term.is_string_sort s)
            ...
            then
              ...
              constr |> Formula.elim_ite
              (* this causes a significant performance degradation *)
              |> Evaluator.simplify
            else constr |> Evaluator.simplify
          in
